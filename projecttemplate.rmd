...
Here the cange is color as we go down to up in total electors represents the population growth through out the years, but also we are looking at a decrease in variance as we grow in poulation.
...
top_party_name <- head(Nat_election_party,5)
...
ggplot(top_parties, aes(x=partyabbre)) + 
  ...
  geom_text(stat="count", color="white", vjust = 0.35, hjust=1.5, size=5, aes(y=..count.., label=..count..)) +
  theme(axis.text.x=element_text(angle=90, size=9)) +
  coord_flip() + theme_minimal() +
...
We can see that out of top five parties two parties have the largest share which are the **BJP** and **INC**.
